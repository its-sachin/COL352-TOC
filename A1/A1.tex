...
Also, string $l_1, l_2 \in $ L, $=> \delta(s,l_1) \bigcup F \neq \phi $ and $ \delta(s,l_2)  \bigcup F \neq \phi$.\\
...
Hence, at best we can create NFA with k+1 states having 1 start state (by creating a new start state s and making $\epsilon $ transitions from s to all other states and rest unchannges).\\
\end{enumerate}
