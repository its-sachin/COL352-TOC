...
No, it is not possible to construct an NFA with k states that accepts the above languae. \\
Proving by contradiction.\\
Suppose $\exists $ an NFA N with k states, s.t. L(N) = L, where L is laguage of strings that don't have all the characters from $\Gamma $.\\
Now, consider the string $s = l_1 l_2 ..... l_k$\\
Clearly, $s \notin L$ (as s contains all characters from $\Gamma $).\\
Let $p = \{p_0, p_1, .......p_k\}$ be the run of NFA on $s$.\\
By pegionhole principle, $\exists p_i, p_j \in p $ such that $p_i = p_j \& i \neq j$\\
$=> $ string $s $ and string $s' = l_1 l_2 .... l_i, l_{j+1}.....l_k$ will have same final state\\
$=> $ N either accepts both $s,s'$ or neither of them.\\
That is a contradiction, since $s \notin L$ but $s' \in L$\\
Hence proved.
